...
# PERBAIKAN: Fungsi simpan_data dan data_tabel_diklik sudah diperbaiki
# agar tidak tertukar.
...
class KonsumenWindow(QWidget):
    ...
    def hubungkan_tombol(self):
        # Menghubungkan tombol-tombol dari .ui baru
        try:
    ...
    def simpan_data(self):
        ...
        nm_perusahaan = self.ui.lineEdit_NamaPerusahaan.text() # <-- Mengambil dari Nama Perusahaan
        nm_pemilik = self.ui.lineEdit_NamaPemilik.text()       # <-- Mengambil dari Nama Pemilik
        alamat = self.ui.textEdit_Alamat.toPlainText()
        telp_fax = self.ui.lineEdit_Telpon.text()
        # ==================================
    ...
    def data_tabel_diklik(self, row, column):
        ...
        self.ui.lineEdit_IdPelanggan.setReadOnly(True) # ID tidak boleh diubah
        ...
        alamat_text = self.ui.tableWidget.item(row, 3).text()
        self.ui.textEdit_Alamat.setPlainText("" if alamat_text == "None" else alamat_text)

        telp_text = self.ui.tableWidget.item(row, 4).text()
        self.ui.lineEdit_Telpon.setText("" if telp_text == "None" else telp_text)
        # ==================================

    def hapus_data(self):
        # Mengambil ID Pelanggan dari form
        id_pel = self.ui.lineEdit_IdPelanggan.text()
        if not id_pel:
